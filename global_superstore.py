...
global_sales_df = pd.read_excel(data_set,0)
...
app.layout = html.Div([
                ...
                html.Div([
                        html.Div([
                                dcc.Dropdown(id='customers-option',options = [{'label': cust,'value': cust} for cust in global_sales_df['Customer Name'].unique()])
                                ],className = 'two columns'),
                        html.Div([
                                html.Button(id='monthly-sale-by-cat-sumbit',n_clicks=0,children='submit')
                                ],className = 'one columns'),
                        html.Div([
                                #html.H1('1.3')
                                ],className = 'nine columns')
                ...
                html.Div([
                        html.H3('Section 3'),
                        dcc.Graph(id = 'graph-grouped-bar-cust-sale-by-month-and-category')
                        ],className="row"),
                html.Div([
                        html.Div([
                                html.H4('Section 4.1'),
...
###################  
@app.callback(dash.dependencies.Output('graph-grouped-bar-cust-sale-by-month-and-category','figure'),
              [dash.dependencies.Input('monthly-sale-by-cat-sumbit','n_clicks')],
              [dash.dependencies.State('customers-option','value')]
             )
def update_cust_sale_by_mon_and_cat(n_clicks,input_value):
    print(f'input value = {input_value}')
    df = pd.read_excel(data_set,0,usecols=['Customer Name','Category','Order Date','Sales'], parse_dates=['Order Date'])
    df_cust = df[df['Customer Name'] == input_value]
    del df
    
    df_cust_pivot_table = df_cust.pivot_table(index = pd.Grouper(freq='M',key = 'Order Date'),
                                              columns='Category',aggfunc='sum',
                                              fill_value=0)
    df_cust_pivot_table_flattened = pd.DataFrame(df_cust_pivot_table.to_records())
    df_cust_pivot_table_flattened.columns =['Order Date','Furniture','Office Supplies','Technology']
    df_cust_pivot_table_flattened['Order Month Year'] = df_cust_pivot_table_flattened['Order Date'].dt.strftime('%B,%Y')
    df_cust_pivot_table_flattened
    print('--------')
    print(df_cust_pivot_table_flattened)
    data = []
    for cat in ['Furniture','Office Supplies','Technology']:
        list_sales = [round(x,2) for x in list(df_cust_pivot_table_flattened[cat])]
        cust_sale_by_cat = go.Bar(
                                  x = list(df_cust_pivot_table_flattened['Order Month Year']),
                                  y = list(df_cust_pivot_table_flattened[cat]),
                                  text = list_sales,
                                  marker = get_marker(False),
                                  textposition = 'inside',
                                  textfont=get_text_font(),
                                  name = cat
                                  )
                           
        data.append(cust_sale_by_cat)


    del df_cust_pivot_table_flattened
    layout = get_graph_layout(graphTitle = f'Sales by month and category for {input_value}',
                              graphWidth = 2000,
                              graphHeight = 1000,
                              graphHoverMode = 'closest',
                              marginLeft = 250,
